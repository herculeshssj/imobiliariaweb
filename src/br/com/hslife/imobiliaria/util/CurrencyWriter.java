...
public class CurrencyWriter {
	...
	public String write(final BigDecimal amount) {
		...
		if (MAX_SUPPORTED_VALUE.compareTo(value) < 0) {
			throw new IllegalArgumentException(
					"Valor acima do limite suportado.");
		}
		 ...
		 */
		int expoente = 3 * (decomposed.size() - 2); // TODO usar um índice de
													// grupos ao invés do
													// expoente
		...
		while (!decomposed.empty()) {
			...
			switch (expoente) { // TODO ao invés desses switches e ifs, partir
								// para a idéia das "Pendências"; talvez
								// implementá-las com enum
			case 0:
	...
	/*
	 * Método modificado para não imprimir "real", "reais", "centavo" e "centavos". 
	 * 
	 * @author Hércules
	 */
	public String writeNoCurrency(final BigDecimal amount) {
		if (null == amount) {
			throw new IllegalArgumentException();
		}
